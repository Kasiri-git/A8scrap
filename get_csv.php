<?php
// get_csv.php: 画像リンクのプログラムIDと掲載URLをCSVで出力する処理
...
if (!empty($links)) {
    // HTMLテーブルを解析してプログラムIDと掲載URLを抽出する
    $dom = new DOMDocument();
    ...
    foreach ($rows as $row) {
        ...
        if ($tds->length === 4) {
            ...
            if (!empty($program_id)) {
                ...
                $csv_data[] = array($program_id, $page_url);
            }
        }
    }
    ...
    header('Content-Disposition: attachment; filename=images_links.csv');
